...
router.put('/:id', async (req, res) => {
  // COMPLETED: Implement UPDATE operation for one post.
...
router.delete('/:id', async (req, res) => {
  // COMPLETED: Implement DELETE operation for one post.
  // 1) Keep ObjectId validation for req.params.id.
  ...
  try {
    ...
    if (!post) {
      return res.status(404).json({ error: 'No Blog post found with this id' });
    }
